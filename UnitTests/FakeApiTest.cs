...
public class FakeApiTest {
    ...
    public async Task List() {
        Result<IReadOnlyList<Station>> result = await _client.ListStationsAsync(21, 12, 5);
        Assert.Multiple(() => {
    ...
    public async Task Details() {
        Result<DetailedStation> result = await _client.GetStationAsync(Guid.Parse("accbd20e-a086-4dd3-a2e9-e8e0978a4f64"));
        Assert.Multiple(() => {
            Assert.That(result.TryGetError(out Exception? exception), Is.False, $"Result Not Success: {exception?.GetType()} - {exception?.Message}");
            Assert.That(result.TryGetValue(out DetailedStation? station), Is.True);
            ...
            Assert.That(station.OpeningTimes[0].Start, Is.EqualTo(new TimeOnly(6, 0)));
            Assert.That(station.OpeningTimes[0].End, Is.EqualTo(new TimeOnly(22, 0)));
            Assert.That(station.OpeningTimes[1].Start, Is.EqualTo(new TimeOnly(7, 0)));
            Assert.That(station.OpeningTimes[1].End, Is.EqualTo(new TimeOnly(22, 0)));
            Assert.That(station.OpeningTimes[2].Start, Is.EqualTo(new TimeOnly(8, 0)));
            Assert.That(station.OpeningTimes[2].End, Is.EqualTo(new TimeOnly(22, 0)));
            Assert.That(station.WholeDay, Is.False);
        });
    }
}